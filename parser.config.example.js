module.exports = {
  kaspi: {
    ...
    // Если магазин есть в этом списке, парсер считает его "своим",
    // не воспринимает как внешнего конкурента и использует это в статистике.
  ...
  notifications: {
    ...
    telegram: {
      ...
      // Кого тегать в аварийных уведомлениях.
      // Используется для падений парсера и проблем со стартом Kaspi.
      // Указывай Telegram username с @.
      adminMentions: ['@admin1', '@admin2'],
    },
  },
  statistics: {
    // Базовая папка статистики.
    // Внутри неё проект хранит один файл SQLite `statistics.sqlite`.
    // Если в `runs/` лежат старые JSON-прогоны, они будут один раз импортированы в базу.
    storageDir: 'data/statistics',
    ui: {
      // Порт локального интерфейса просмотра статистики.
      port: 3080,
    },
  },
  pricing: {
